...
const SvgBuilder = {
    createRoot: (w, h) => {
        ...
        // 建立坐标系：中心点为 (0,0)，宽高 800
        svg.setAttribute("viewBox", `${-w / 2} ${-h / 2} ${w} ${h}`);
        svg.style.display = "block";
        return svg;
    },
    createCircle: (r, fill, strokeClass) => {
        const c = document.createElementNS(SVG_NS, "circle");
        c.setAttribute("cx", 0); c.setAttribute("cy", 0); c.setAttribute("r", r);
        if (fill) c.setAttribute("fill", fill);
        if (strokeClass) c.setAttribute("class", strokeClass);
        return c;
    },
    createLine: (x1, y1, x2, y2, strokeClass) => {
        const l = document.createElementNS(SVG_NS, "line");
        l.setAttribute("x1", x1); l.setAttribute("y1", y1); l.setAttribute("x2", x2); l.setAttribute("y2", y2);
        l.setAttribute("class", strokeClass || "");
        return l;
    },
    createText: (txt, x, y, color, size, isBold) => {
        const t = document.createElementNS(SVG_NS, "text");
        t.setAttribute("x", x); t.setAttribute("y", y);
        t.setAttribute("class", "text-planet");
        t.setAttribute("fill", color);
        t.setAttribute("font-size", size);
        if (isBold) t.style.fontWeight = "bold";
        t.textContent = txt;
        return t;
    }
};
...
window.renderAstroChart = function (CHART_DATA) {
    try {
        ...
        const FONTS = STYLE.fonts; // 虽然 SVG 用不到 Canvas Font 字符串，但我们会参考大小
        ...
        if (!container) {
            console.error("❌ 找不到 ID 为 'svgChartContainer' 的元素");
            return;
        }
        container.innerHTML = ''; // 清空旧内容

        // 3. 创建 SVG 画布 (800x800)
        const svg = SvgBuilder.createRoot(930, 930);
        ...
        // 核心：计算屏幕坐标 (替代 ctx.rotate/translate)
        // 升宫 (ASC) 在左边 (PI/180度)
        function getPos(objLon, radius) {
            const ascLon = CHART_DATA.asc_lon;
            const angle = Math.PI + deg2rad(ascLon - objLon);
            return {
                x: Math.cos(angle) * radius,
                y: Math.sin(angle) * radius,
                angle: angle // 保留弧度供逻辑使用
            };
        }
        ...
        // 1. 绘制同心圆 (背景填充)
        // SVG 是层叠的，先画大的在底下
        svg.appendChild(SvgBuilder.createCircle(RADII.r1, ZONES.outerRing));
        svg.appendChild(SvgBuilder.createCircle(RADII.r3, ZONES.planetRing));
        svg.appendChild(SvgBuilder.createCircle(RADII.r9, ZONES.innerDisk));
        svg.appendChild(SvgBuilder.createCircle(RADII.r11, ZONES.centerCore));

        // 2. 绘制宫位分割线
        CHART_DATA.houses.forEach(h => {
            const p1 = getPos(h.abs_lon, RADII.r3);
            const p2 = getPos(h.abs_lon, RADII.r11);
            // 使用 CSS 类 .stroke-house
            svg.appendChild(SvgBuilder.createLine(p1.x, p1.y, p2.x, p2.y, "stroke-house"));
        });

        // 3. 绘制圆环边界线 (描边)
        [RADII.r1, RADII.r3, RADII.r9, RADII.r11].forEach(r => {
            // fill="none", 使用 CSS 类 .stroke-main
        ...
        renderPlanets.forEach(p => {
            // 使用防碰撞后的 render_lon
            const pLon = p.render_lon;
            const z = p.zodiac;
            const style = getPlanetStyle(p.name);
            const signColor = getSignColor(z.sign_idx);

            // A. 行星符号 (r4)
            const pSym = getPos(pLon, RADII.r4);
            svg.appendChild(SvgBuilder.createText(style.symbol, pSym.x, pSym.y, style.color, "28px"));

            // B. 度数 (r5)
            const pDeg = getPos(pLon, RADII.r5);
            svg.appendChild(SvgBuilder.createText(z.deg + "°", pDeg.x, pDeg.y, COLORS.textMain, "25px"));

            // C. 星座符号 (r6)
            const pZod = getPos(pLon, RADII.r6);
            svg.appendChild(SvgBuilder.createText(z.sign_sym, pZod.x, pZod.y, signColor, "20px"));

            // D. 分钟 (r7)
            const pMin = getPos(pLon, RADII.r7);
            svg.appendChild(SvgBuilder.createText(z.min + "′", pMin.x, pMin.y, COLORS.textDim, "25px"));

            // E. 逆行标记 (r8)
            if (p.is_retro) {
                const pRet = getPos(pLon, RADII.r8);
                const t = SvgBuilder.createText("R", pRet.x, pRet.y, COLORS.retro, "21px", true);
                t.setAttribute("class", "text-retro");
                svg.appendChild(t);
            }
        });
        ...
        if (CHART_DATA.house_mids) {
            CHART_DATA.house_mids.forEach(hm => {
                const pNum = getPos(hm.lon, RADII.r10);
                const t = SvgBuilder.createText(hm.id, pNum.x, pNum.y, COLORS.highlight, "18px", true);
                svg.appendChild(t);
            });
        }

        console.log("✅ SVG 星盘绘制成功");

    } catch (err) {
        console.error("❌ SVG 绘制过程出错:", err);
    }
};
...
window.renderAspectTables = function (chartData) {
    ...
    const buildMatrixHTML = (modeName, aspectList, axisItems) => {
        ...
        // 2. 计算网格列数
        // 结构：[左侧标签列] + [N 个数据/对角线列]
        // 例如：axisItems 有 10 个行星，那么总列数 = 1 + 10 = 11
        const totalCols = axisItems.length + 1;

        // 3. 开始构建 HTML
        // 注意：我们在 style 里动态写入 grid-template-columns
        // 第一列给 auto (适应标签宽)，后面 N 列平分 (1fr)
        let html = `
            <div class="aspect-mode-block">
                <div class="aspect-mode-title">✨ ${modeName}</div>
                <div class="aspect-grid-mode" 
                     style="grid-template-columns: auto repeat(${axisItems.length}, 1fr);">
        `;

        // 4. 双层循环生成“扁平”的格子流
        // Grid 布局不需要 <div>row</div>，直接塞 cell 进去就行
        for (let i = 0; i < axisItems.length; i++) {
            const currentItem = axisItems[i];
            const currentSym = getSymbol(currentItem);

            // --- A. 左侧标签列 (Left Label) ---
            if (i === 0) {
                // 第一行左上角通常是空的，或者放个Logo? 这里保持原来的逻辑：隐形
                html += `<div class="aspect-cell aspect-label"></div>`;
            } else {
                html += `<div class="aspect-cell aspect-label">${currentSym}</div>`;
            }

            // --- B. 遍历右侧 N 列 (数据区) ---
            for (let j = 0; j < axisItems.length; j++) {
                // 这里的 j 代表列索引 (对应 targetItem)
                ...
                if (j < i) {
                    ...
                    if (data) {
                        ...
                        html += `<div class="aspect-cell">${content}</div>`;
                    } else {
                        // 无相位
                        html += `<div class="aspect-cell"></div>`;
                    }

                } else if (j === i) {
                    // [情况 2] 对角线 (Row == Col) -> 显示标签
                    html += `<div class="aspect-cell aspect-label">${currentSym}</div>`;

                } else {
                    // [情况 3] 上三角 (Row < Col) -> 以前是不渲染，现在必须渲染“空格子”占位
                    html += `<div class="aspect-cell aspect-empty"></div>`;
                }
            }
        }
