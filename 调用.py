...
# ----------------- 设置所有占星参数 -----------------
chart_name = {"matter_name": "澳大利亚留学女朋友"}

birth_config = {
    "local_time_str": "2026-02-12 20:51:37.000000", # 本地时间（必须含两位小数秒）
    "timezone_str": "+8:00", # 时区（支持+8、-5:30、+5.5格式）
    "latitude_str": "23°02′00.000000″", # 纬度（必须带两位小数秒）
    "longitude_str": "113°06′00.000000″", # 经度（必须带两位小数秒）
...
calculation_options = {
    ...
    "house_system": 'Regiomontanus', # 宫位制：[Placidus|Koch|Regiomontanus|Whole Sign|Equal|Campanus]
    ...
    "number": 53 # "KS-N"对应1-249之间整数，"1-2193"对应1-2193之间整数
    }
}
...
print("\n" + "🚀 相位计算结果 (Aspects Report)".center(60, "="))
...
if 'orb_mode' in aspect_results and aspect_results['orb_mode']:
    ...
    for row in aspect_results['orb_mode']:
        ...
        # 精度控制
        actual = f"{row['actual_dist']:.4f}°" # 实际角度
        orb_val = f"{row['orb']:.6f}°"        # 容许度误差
        defn = f"{row['angle_def']:.0f}°"     # 定义角度(如90)
...
if 'whole_sign_mode' in aspect_results and aspect_results['whole_sign_mode']:
    ...
    print(f"\n[🏛️ 整宫制模式 (Whole Sign)] - 共 {count} 条记录")
    print_separator(40)
...
if 'vedic_mode' in aspect_results and aspect_results['vedic_mode']:
    ...
    print(f"\n[🕉️ 印度模式 (Vedic Mode)] - 共 {count} 条视线")
    print_separator(50)
    print(f"{'Caster (施)':<12} {'Type':^10} {'Receiver (受)':<12}")
    print_separator(50)
